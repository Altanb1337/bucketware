...
namespace ENet.Managed
{
    public sealed unsafe class ENetHost : DisposableBase
    {
        ...
        [SecurityCritical]
        public unsafe ENetHost(IPEndPoint address, int peers, byte channels, long incomingBandwidth = 0, long outgoingBandwidth = 0)
        {
            ...
            {
                var nativeAddress = NativeENetAddress.FromIPEndPoint(address);
                m_Pointer = LibENet.HostCreate(nativeAddress.Type, null, (UIntPtr)peers, (UIntPtr)channels, (uint)incomingBandwidth, (uint)outgoingBandwidth);
            }
        ...
        public void EnableNewProtocol(int state)
        {
            CheckDispose();
            Console.WriteLine(state);
            Console.WriteLine(m_Pointer);
            LibENet.HostEnableNewProtocol(m_Pointer, state);
        }
        ...
        private ManualResetEvent m_ServiceThreadStopEvent;
        private Thread m_ServiceThread;
        private CancellationTokenSource m_ServiceThreadStopToken;
